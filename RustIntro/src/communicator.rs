...
impl Command {
    pub fn as_str (&self) -> String {
        ...
        match self {
            ...
            Command::Shield(true) => {
                str = String::from("Shield turned on");
            },
            Command::Shield(false) => {
                str = String::from("Shield turned off");
            },
            Command::Try => {
                str = String::from("Call attempt failed");
            },
            Command::Invalid => {
                str = String::from("Not a command");
            },
        };
        str
    }
...
pub fn to_command(s: &str) -> Command {
    ...
    if v.len() == 3 && v[0] == "power"{
        ...
        if v[1] ==  "dec" && b {
            cmd = Command::Power(false,v[2].parse::<i32>().unwrap());
        }
        else if v[1] ==  "inc" && b{
            cmd = Command::Power(true,v[2].parse::<i32>().unwrap());
        }
        ...
        if v.len() == 3 && v[2] == "missiles" && c {
            cmd = Command::Missiles(true,v[1].parse::<i32>().unwrap());
        }
        ...
        if v.len() == 3 && v[2] == "missiles" && d {
            cmd = Command::Missiles(false,v[1].parse::<i32>().unwrap());
        }
        ...
        if v.len() == 4 && v[0] == "try" && v[1] == "calling" && v[2] == "Miss" && v[3] == "Potts"{
            cmd = Command::Try;
        }
    } else {
        cmd = Command::Invalid;
